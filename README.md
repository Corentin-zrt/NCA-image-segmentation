...
## Version 2 CPU / Cityscapes

La baseline precedente est archivee dans `legacy/v1_synthetic_numpy/`.
L'infrastructure v2 se trouve dans `src/nca_segmentation/v2/` et utilise
PyTorch directement dans l'interpreteur Python systeme, sans environnement
virtuel :
...
Le dataset Cityscapes doit etre telecharge par l'utilisateur selon sa licence.
Place ses dossiers `leftImg8bit/` et `gtFine/` dans un repertoire local, puis
utilise `CityscapesInstances` depuis `nca_segmentation.v2.data`.

Le modele v2 combine un encodeur convolutionnel, un NCA recurrent et trois
sorties : classes semantiques, objectness et embeddings d'instances.
Les checkpoints sont des fichiers `.pt` dans `artifacts/runs/v2/`.

Pour lancer un entraînement réel après avoir placé Cityscapes dans un dossier
local :

```powershell
python scripts/train_cityscapes.py --root D:\datasets\cityscapes --epochs 10
```

Le script utilise `num_workers=0` et le CPU par défaut. La v1 se lance depuis
`legacy/v1_synthetic_numpy/` avec sa propre commande `python main.py`.
